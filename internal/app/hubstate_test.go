...
func TestHubState_ErrorsAndAbsentHome(t *testing.T) {
	t.Run("cancelled context is reported when reading", func(t *testing.T) {
		...
		cancel()
		if _, _, err := HubState(ctx, state.HubStateTourStepKey("install-suite")); err == nil {
	...
	t.Run("cancelled context is reported when deleting", func(t *testing.T) {
		...
		ctx, cancel := context.WithCancel(context.Background())
		cancel()
		if err := DeleteHubState(ctx, state.HubStateTourStepKey("install-suite")); err == nil {
	...
	t.Run("a relative home is refused rather than guessed at", func(t *testing.T) {
		...
		if _, _, err := HubState(context.Background(), state.HubStateTourStepKey("install-suite")); err == nil {
	...
	t.Run("a home with no database reports the key as absent", func(t *testing.T) {
		t.Setenv("FESTIVAL_HOME", t.TempDir())
		value, ok, err := HubState(context.Background(), state.HubStateTourStepKey("install-suite"))
		if err != nil {
	...
	t.Run("deleting from a home with no database does nothing", func(t *testing.T) {
		...
		if err := DeleteHubState(context.Background(), state.HubStateTourStepKey("install-suite")); err != nil {
...
func TestHubState_WriteReadDeleteAcrossProcesses(t *testing.T) {
	...
	if err := SetHubState(ctx, state.HubStateTourStepKey("install-suite"), "2"); err != nil {
		t.Fatalf("SetHubState: %v", err)
	}

	value, ok, err := HubState(ctx, state.HubStateTourStepKey("install-suite"))
	if err != nil {
		t.Fatalf("HubState: %v", err)
	}
	if !ok || value != "2" {
	...
	if err := SetHubState(ctx, state.HubStateTourStepKey("install-suite"), "3"); err != nil {
		t.Fatalf("second SetHubState: %v", err)
	}
	value, _, err = HubState(ctx, state.HubStateTourStepKey("install-suite"))
	if err != nil {
	...
	if _, ok, err = HubState(ctx, state.HubStateTourStepKey("install-suite")); err != nil {
